...
class _PdfMakerScreenState extends State<PdfMakerScreen> {
  ...
  List<Map<String, dynamic>> latterDetectionInfos = [
    {
      'detectionInfoId': '507',
      'part': 'front',
      'damageDate': '2023-04-06',
      'memo': '살려줘',
      'damageImageUrl':
          'https://herosbucket.s3.ap-northeast-2.amazonaws.com/hero/user_j y_2023-04-06_13-22-30_1.jpg',
      'former': false,
      'scratch': 0,
      'breakage': 3,
      'crushed': 4,
  ...
  Future<List<Map<String, dynamic>>> changeImageToMemory(
      String dataCase) async {
    ...
    if (widget.detailRentInfo[dataCase].isNotEmpty) {
      for (int i = 0; i < widget.detailRentInfo[dataCase].length; i++) {
        final response = await http.get(
            Uri.parse(widget.detailRentInfo[dataCase][i]['damageImageUrl']));
        ...
        Map<String, dynamic> damageInfo = {
          ...
          'memo': widget.detailRentInfo[dataCase][i]['memo'] == ''
              ? '없음'
              : widget.detailRentInfo[dataCase][i]['memo'],
  ...
  Future<Uint8List> _generatePdf(PdfPageFormat format) async {
    final pdfTheme = pw.ThemeData.withFont(
      base: pw.Font.ttf(
          await rootBundle.load('lib/assets/fonts/Pretendard-Regular.ttf')),
      bold: pw.Font.ttf(
          await rootBundle.load('lib/assets/fonts/Pretendard-Bold.ttf')),
    );
    final pdf = pw.Document(
        version: PdfVersion.pdf_1_5, compress: true, theme: pdfTheme);
    final pageFormat = PdfPageFormat.a4;
    final logoImage =
        await rootBundle.load('lib/assets/images/logo/reccar_logo_png_ver.PNG');
      ...
      build: (context) => [
        for (final data in finalInfoList)
          pw.Column(
            ...
            children: [
              pw.Text(
                data['former'] ? '대여 직후 손상 상세' : '반납 직전 손상 상세',
                style: pw.TextStyle(
                  fontSize: 20,
                  fontWeight: pw.FontWeight.bold,
                ),
